...
void D3D11VertexBuffer::FlushBuffers()
{
    ...
    if(FAILED(err = GetD3DContext()->Map(vertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map)))
    {
        AppWarning(TEXT("D3D11VertexBuffer::FlushBuffers: failed to map vertex buffer, result = %08lX"), err);
        return;
    }

    if(App->SSE2Available())
        SSECopy(map.pData, data->VertList.Array(), sizeof(Vect)*numVerts);
    else
    ...
    if(normalBuffer)
    {
        if(FAILED(err = GetD3DContext()->Map(normalBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map)))
        {
            AppWarning(TEXT("D3D11VertexBuffer::FlushBuffers: failed to map normal buffer, result = %08lX"), err);
            return;
        }

        if(App->SSE2Available())
            SSECopy(map.pData, data->NormalList.Array(), sizeof(Vect)*numVerts);
        else
            mcpy(map.pData, data->NormalList.Array(), sizeof(Vect)*numVerts);

        GetD3DContext()->Unmap(normalBuffer, 0);
    }
    ...
    if(colorBuffer)
    {
        if(FAILED(err = GetD3DContext()->Map(colorBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map)))
        {
            AppWarning(TEXT("D3D11VertexBuffer::FlushBuffers: failed to map color buffer, result = %08lX"), err);
            return;
        }

        if(App->SSE2Available())
            SSECopy(map.pData, data->ColorList.Array(), sizeof(Vect)*numVerts);
        else
            mcpy(map.pData, data->ColorList.Array(), sizeof(Vect)*numVerts);

        GetD3DContext()->Unmap(colorBuffer, 0);
    }
    ...
    if(tangentBuffer)
    {
        if(FAILED(err = GetD3DContext()->Map(tangentBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map)))
        {
            AppWarning(TEXT("D3D11VertexBuffer::FlushBuffers: failed to map tangent buffer, result = %08lX"), err);
            return;
        }

        if(App->SSE2Available())
            SSECopy(map.pData, data->TangentList.Array(), sizeof(Vect)*numVerts);
        else
            mcpy(map.pData, data->TangentList.Array(), sizeof(Vect)*numVerts);

        GetD3DContext()->Unmap(tangentBuffer, 0);
    }
    ...
    if(UVBuffers.Num())
    {
        for(UINT i=0; i<UVBuffers.Num(); i++)
        {
            ...
            zero(&map, sizeof(map));
            if(FAILED(err = GetD3DContext()->Map(buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map)))
            {
                AppWarning(TEXT("D3D11VertexBuffer::FlushBuffers: failed to map texture vertex buffer %d, result = %08lX"), i, err);
                return;
            }

            if(App->SSE2Available())
                SSECopy(map.pData, textureVerts.Array(), sizeof(UVCoord)*numVerts);
            else
...
VBData* D3D11VertexBuffer::GetData()
{
    if(!bDynamic)
    {
        AppWarning(TEXT("D3D11VertexBuffer::FlushBuffers: Cannot get vertex data of a non-dynamic vertex buffer"));
        return NULL;
    }
